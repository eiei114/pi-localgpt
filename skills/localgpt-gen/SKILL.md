...
| 状況 | Tool |
|------|------|
| 過去の記憶を検索 | `localgpt_memory_search` |
| 検索結果の詳細を読む | `localgpt_memory_get` |
| 長期記憶として保存 | `localgpt_memory_save` |
| 日次ログに追記 | `localgpt_memory_log` |
...
```
localgpt_gen_plan          → BlockoutSpec JSON
...
1. 作業開始 → `localgpt_memory_search` で過去の記録を確認
2. テーマ・スタイル決定 → `localgpt_memory_save`
3. セッション記録 → `localgpt_memory_log`
